...
@click.option(
    "--engine",
    "-e",
    type=click.Choice(["auto", "arxiv", "web"]),
    default="auto",
...
def crawl(ctx, target, output, format, storage, config, engine, async_mode):
    ...
    try:
        ...
        storage_config = {"type": storage, "format": format}
        if output:
        ...
        if engine == "auto":
            ...
            result = asyncio.run(
                _async_crawl(factory, target, storage_adapter, ctx, engine)
            )
        else:
...
def arxiv(ctx, arxiv_input, output, format, max_results, sort_by, category):
    ...
    try:
        ...
        crawl_params = {"max_results": max_results, "sort_by": sort_by}
        if category:
        ...
        if output:
            ...
            # Generate output filename
            safe_input = "".join(
                c for c in arxiv_input if c.isalnum() or c in ("-", "_")
            )[:50]
...
@click.option(
    "--max-links", type=int, default=100, help="Maximum number of links to extract"
)
...
@click.option(
    ...
    "-s",
    is_flag=True,
    help="Treat target as a search query instead of a URL",
)
@click.pass_context
def web(
    ctx,
    ...
    try:
        ...
        if not ctx.obj.get("quiet"):
            ...
            if result.get("type") == "web_search_results":
                ...
                click.echo(
                    f"Extracted {text_length} characters of text and {links_count} links"
                )
...
@click.option(
    "--mode",
    "-m",
    type=click.Choice(["full", "abnormal", "filter", "privacy", "no_html"]),
    default="full",
    help="Cleaning mode",
)
@click.option(
...
@click.option(
    "--min-chars", type=int, default=30, help="Minimum character count for filtering"
)
@click.option(
    "--max-chars",
    type=int,
        ...
        if mode == "full":
            ...
            if isinstance(cleaned, dict) and "text" in cleaned and cleaned["text"]:
                ...
                if (
                    ...
                    if not ctx.obj.get("quiet"):
                        click.echo(
                            "Warning: Text failed filtering, skipping privacy cleaning"
                        )
                    result = cleaned
            else:
                result = cleaned
...
def list_generators(ctx):
    ...
    try:
        ...
        click.echo(
            "   datamax generator multimodal document.md --api-key $OPENAI_API_KEY"
        )
    ...
    try:
        ...
        if use_mineru and use_qwen_vl_ocr:
            raise click.BadParameter(
                "Cannot use both --use-mineru and --use-qwen-vl-ocr simultaneously"
            )

        if use_mllm and (use_mineru or use_qwen_vl_ocr):
            raise click.BadParameter(
                "Cannot use --use-mllm with PDF options (--use-mineru/--use-qwen-vl-ocr)"
            )

        # Generate output file path if not provided
        if not output_file:
            input_path = Path(input_file)
            extension = {"markdown": "md", "json": "json", "text": "txt"}.get(
                format, "md"
            )
        ...
        result = parser.parse_file(
            ...
            use_mllm=use_mllm,
            mllm_system_prompt=mllm_prompt
            or "描述图片内容，包括图片中的文字、图片中的对象、图片中的场景等。输出一份专业的中文markdown报告",
            api_key=api_key,
            base_url=base_url,
            model_name=model,
...
def batch(
    ...
    try:
        ...
        if not ctx.obj.get("quiet"):
            ...
            click.echo(
                f"Pattern: {pattern}, Recursive: {recursive}, Workers: {max_workers}"
            )
...
def list_formats(ctx):
    ...
    try:
        ...
        click.echo(
            "   datamax parser parse image.jpg --use-mllm --api-key $OPENAI_API_KEY"
        )
