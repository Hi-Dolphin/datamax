...
class PdfLLMOcrProcessor(BaseLife):
    ...
    def read_pdf_file(file_path) -> str:
        try:
            ...
            result_text = "".join(page.page_content for page in pdf_documents)
            return result_text
...
class PdfParser(BaseLife):
    ...
    #  Unified parse()
    # ============================================================
    def parse(self, file_path: Union[str, List[str]]):
        """
        Main parse entry (low complexity):
        - Handles list → _parse_multi
        - Handles single → _parse_single
        """
        if isinstance(file_path, list):
            return self._parse_multi(file_path)
        return self._parse_single(file_path)

    # ============================================================
    #  Parse multiple files
    # ============================================================
    def _parse_multi(self, file_list: List[str]):
        results = []
        for f in file_list:
            try:
                results.append(self._parse_single(f))
            except Exception as e:
                lc_fail = self.generate_lifecycle(
                    source_file=f,
                    domain=self.domain,
    ...
    def _parse_single(self, file_path: str):
        ...
        try:
            mk_content, extra_lifecycle = self._process_pdf_content(file_path)
            ...
            output_vo = MarkdownOutputVo("md", mk_content)
            output_vo.add_lifecycle(lc_start)

            # Add per-page lifecycle (only Qwen OCR)
            for lc in extra_lifecycle:
                output_vo.add_lifecycle(lc)
    ...
    # ============================================================
    #  Core PDF content processing logic
    # ============================================================
    def _process_pdf_content(self, file_path: str):
        """
        Returns:
            mk_content(str), lifecycle(list)
        """
        # -------------------------------
        # Qwen-VL OCR
        # -------------------------------
        if self.use_qwen_vl_ocr:
            ...
            if isinstance(result, dict):
                ...
                lifecycle_items = result.get("lifecycle", [])
            else:
                mk_content = getattr(result, "content", str(result))
                lifecycle_items = getattr(result, "lifecycle", [])

            return mk_content, lifecycle_items
        ...
        # -------------------------------
        mk_content = self.read_pdf_file(file_path)
        return mk_content, []
