...
from .exceptions import ConfigurationException, CrawlerException
    ...
    def register_crawler(
        ...
        if not issubclass(crawler_class, BaseCrawler):
            raise CrawlerException(
                f"Crawler class must inherit from BaseCrawler: {crawler_class}"
            )
...
def _register_default_crawlers(factory: CrawlerFactory):
    ...
    try:
        ...
        factory.register_crawler(
            "arxiv", ArxivCrawler, [r"arxiv\.org", r"export\.arxiv\.org"]
        )
    except ImportError:
    ...
    try:
        ...
        factory.register_crawler("web", WebCrawler, [r"https?://.*"])
    except ImportError:
