    ...
    async def _fetch_paper_metadata(self, arxiv_id: str) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise NetworkException(
                        f"Failed to fetch ArXiv paper {arxiv_id}: {str(e)}"
                    )
        ...
        try:
            ...
            async with session.get(self.api_url, params=params) as response:
                if response.status == 200:
                    ...
                    raise NetworkException(
                        f"ArXiv search API returned status {response.status}"
                    )
    ...
    async def crawl_async(self, target: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if arxiv_id:
                ...
                return {"type": "single_paper", "target": target, "data": paper_data}
            else:
                # Search query
                max_results = kwargs.get(
                    "max_results", self.config.get("search_max_results", 10)
                )
