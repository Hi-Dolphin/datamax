...
DEFAULT_DATABASE_URLS = {"development": "sqlite:///./datamax-dev.db", "test": "sqlite:///./datamax-test.db", "production": None}  # Must be provided
...
class MigrationManager:
    ...
    def _get_applied_migrations(self) -> List[str]:
        ...
        try:
            with self.engine.connect() as conn:
                result = conn.execute(text(f"SELECT version FROM {MIGRATIONS_TABLE} ORDER BY version"))
        ...
        try:
            with self.engine.connect() as conn:
                conn.execute(
                    ...
                    {"version": migration.version, "name": migration.name, "applied_at": migration.timestamp},
                )
                conn.commit()
        except Exception as e:
    ...
    def _mark_migration_reverted(self, version: str):
        ...
        try:
            with self.engine.connect() as conn:
                conn.execute(text(f"DELETE FROM {MIGRATIONS_TABLE} WHERE version = :version"), {"version": version})
                conn.commit()
        except Exception as e:
    ...
    def _execute_sql(self, sql: str, migration_name: str = None):
        ...
        try:
            ...
            error_msg = "Failed to execute SQL"
            if migration_name:
    ...
    def _execute_python(self, func, migration_name: str = None):
        ...
        try:
            ...
            error_msg = "Failed to execute Python migration"
            if migration_name:
...
def get_database_url(args) -> str:
    ...
    if not default_url:
        raise MigrationError(f"No database URL configured for environment '{env}'. " "Please provide --database-url or set DATABASE_URL environment variable.")

    return default_url


def print_status(status: Dict):
    """Print migration status details."""
...
def execute_command(manager: MigrationManager, args: argparse.Namespace) -> int:
    ...
    if args.command == "migrate":
        ...
        migration_file = manager.create_migration(name=args.name, sql=not args.python)
...
Environment Variables:
  ...
  DATAMAX_ENV      Environment name (development, test, production)
    ...
    parser.add_argument("--environment", "-e", choices=["development", "test", "production"], help="Environment name")

    parser.add_argument("--migrations-dir", type=Path, default=MIGRATIONS_DIR, help=f"Migrations directory (default: {MIGRATIONS_DIR})")
    ...
    rollback_group.add_argument("--target", help="Target migration version to rollback to")
    rollback_group.add_argument("--steps", type=int, default=1, help="Number of migrations to rollback (default: 1)")
    ...
    create_parser.add_argument("--python", action="store_true", help="Create Python migration instead of SQL")
    ...
    try:
        ...
        manager = MigrationManager(database_url=database_url, migrations_dir=args.migrations_dir, verbose=args.verbose)

        # Execute command
        return execute_command(manager, args)
