...
class IntermodalityPipeline:
    ...
    def _setup_directories(self):
        ...
        logger.info(f"Ensuring output directories exist in: {self.config.OUTPUT_BASE_DIR}")
        for path in [self.config.RAW_DATA_DIR, self.config.PARSED_MD_DIR,
                     self.config.IMAGES_DIR, self.config.GENERATED_QA_DIR,
    ...
    async def step_1_crawl_data(self) -> list[Path]:
        ...
        try:
            search_results = await crawler.crawl_async(self.config.SEARCH_QUERY, max_results=self.config.MAX_PAPERS_TO_CRAWL)
            if not search_results or not search_results.get('data'):
                logger.warning("Failed to crawl any papers from ArXiv.")
                return []
            papers = search_results['data']
            logger.success(f"Successfully found {len(papers)} relevant papers. Starting download...")
            downloaded_pdf_paths = []
            for paper in papers:
                pdf_url = paper.get('pdf_url')
                arxiv_id = paper.get('arxiv_id', 'unknown_id').replace('.', '_')
                if not pdf_url:
                    logger.warning(f"Paper '{arxiv_id}' does not have a PDF link, skipping.")
                    continue
                ...
                try:
                    ...
                    logger.error(f"An unknown error occurred while processing '{local_pdf_path}': {e}")
    ...
    def step_2_parse_to_multimodal_markdown(self, pdf_paths: list[Path]) -> list[Path]:
        ...
        logger.info("--- Step 2: Starting to parse PDFs into multimodal Markdown with absolute image paths ---")
        md_paths = []
        for pdf_path in pdf_paths:
            try:
                ...
                logger.success(f"Successfully parsed and saved '{pdf_path.name}' to '{md_path}' with absolute image paths.")
            except Exception as e:
                logger.error(f"Failed to parse file '{pdf_path.name}': {e}", exc_info=True)
        return md_paths
    ...
    def step_4_evaluate_and_filter(self, generated_qa: list[dict]) -> tuple[list[dict], list[dict]]:
        ...
        for i, qa_item in enumerate(generated_qa):
            ...
            if not images:
                logger.warning(f"QA pair #{i+1} is missing images, skipping evaluation.")
                continue
        ...
        logger.success(f"Saved {len(high_quality_data)} high-quality data entries to: {final_data_path}")

        return high_quality_data, evaluation_report

    async def run(self):
        logger.info("🚀 Starting the intermodality multimodal data generation and validation pipeline...")

        if not self.config.DASHSCOPE_API_KEY or self.config.DASHSCOPE_API_KEY == "YOUR_DASHSCOPE_API_KEY":
            logger.error("Please set your DASHSCOPE_API_KEY in the script or as an environment variable.")
            return
        ...
        logger.success(f"After evaluation, {total_passed} high-quality QA pairs were selected (Pass rate: {pass_rate:.2f}%).")
        logger.success(f"The final dataset is saved in: {self.config.EVALUATED_DATA_DIR}")
