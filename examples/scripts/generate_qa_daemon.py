...
def fetch_and_filter_obs_files(loader: DataLoader) -> List[str]:
    ...
    try:
        ...
        # On DB failure, fail safe or retry? Here we return empty list to avoid flooding LLM
        return []

    # 4. Filter logic: Skip 'COMPLETED' and 'PROCESSING'
    #    (Optional: could retry 'FAILED' after some time, here we process 'FAILED' again immediately)
    new_keys = []
    for key in all_keys:
...
def process_file(input_file: Path, obs_key: Optional[str] = None) -> bool:
    ...
    :param obs_key: Original OBS key if applicable (for status tracking).
    ...
    try:
        ...
        # Mark as PROCESSING in DB if applicable
        if obs_key and persistence_config:
            p_config = PersistenceConfig.from_mapping(persistence_config)
            with PostgresPersistence(p_config) as p:
                p.upsert_file_status(p_config.source_key, obs_key, "PROCESSING")
        ...
        if not content or not content.strip():
            ...
            # Empty content -> Mark as FAILED or COMPLETED? 
            # Maybe FAILED to allow check later, or COMPLETED to stop retrying empty file.
            # Let's mark COMPLETED to avoid infinite loop on empty file.
            if obs_key and persistence_config:
                p_config = PersistenceConfig.from_mapping(persistence_config)
                with PostgresPersistence(p_config) as p:
                    p.upsert_file_status(p_config.source_key, obs_key, "COMPLETED", error_message="Empty content")
            return True
        ...
        if not qa:
            ...
            # Similar logic to empty content
            if obs_key and persistence_config:
                p_config = PersistenceConfig.from_mapping(persistence_config)
                with PostgresPersistence(p_config) as p:
                    p.upsert_file_status(p_config.source_key, obs_key, "COMPLETED", error_message="No QA generated")
            return True
        ...
        if obs_key and persistence_config:
            p_config = PersistenceConfig.from_mapping(persistence_config)
            with PostgresPersistence(p_config) as p:
                p.upsert_file_status(p_config.source_key, obs_key, "COMPLETED")
        ...
        if obs_key and persistence_config:
            try:
                p_config = PersistenceConfig.from_mapping(persistence_config)
                with PostgresPersistence(p_config) as p:
                    p.upsert_file_status(p_config.source_key, obs_key, "FAILED", error_message=str(e))
            except Exception as db_err:
                logger.error(f"Failed to update DB status for failure: {db_err}")
        return False
    ...
    if qa_input_source == "obs":
        ...
        for key in target_keys:
            try:
                ...
                # If we want to preserve directory structure locally:
                # local_file_path = download_dir / key
                ...
                if loader.obs:
                    loader.obs.download_file(key, str(local_file_path))
